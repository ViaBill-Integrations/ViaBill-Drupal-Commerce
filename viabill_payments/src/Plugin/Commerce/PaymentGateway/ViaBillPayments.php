...
class ViaBillPayments extends OffsitePaymentGatewayBase implements SupportsRefundsInterface, SupportsVoidsInterface {  
  ...
  public function __construct(
    ...
    ConfigFactoryInterface $config_factory
        ...
        parent::__construct($configuration, $plugin_id, $plugin_definition, $entity_type_manager, $payment_type_manager, $payment_method_type_manager, $time, $minor_units_converter, $event_dispatcher);
  ...
  /**
   * Commerce calls buildPaymentOperations() to build the list of available actions for each payment.
   * By default, OffsitePaymentGatewayBase automatically adds “Void” and “Refund” operations if it detects voidPayment() / refundPayment(), but it may not do the same for capturePayment().
   * Overriding buildPaymentOperations() ensures a “Capture” action is added whenever the payment is in authorization.
   * 
	 * {@inheritdoc}
	 */
	public function buildPaymentOperations(PaymentInterface $payment) {
      ...
      $operations['capture'] = [
        ...
        // Tells Commerce which plugin form to display (often "capture-payment" is recognized).
 ...
 */
    ...
    try {
      if ($gateway->captureTransaction($data)) {
        ...
        if ($new_captured_amount->equals($authorized_amount)) {
          ...
          $payment->setState('completed'); // instead of partially_captured
          ...
          // Add permanent activity log to the order (requires commerce_log module)
  ...
  public function onCancel(OrderInterface $order, Request $request) {
    ...
    // Don't cancel the order, just record the cancellation in logs/activity
    // Add an activity log entry (if you have a custom activity logging system)
    // Optionally, you could set a flag on the order to indicate payment was attempted with ViaBill
    ...
    // Redirect to cart or checkout page instead of letting the system redirect back
