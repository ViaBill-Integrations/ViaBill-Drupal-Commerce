...
class ViaBillController extends ControllerBase {
  ...
  public function callback(Request $request) {
    ...
    try {      
      ...
      if (!empty($content) && $content_type === 'application/json') {
        try {
          $data = json_decode($content, true); // instead of $request->request->all();              
        } catch (\Exception $e) {
  ...
  private function processApprovedPayment(OrderInterface $order, $payment_gateway, $transaction_id, $amount, $currency, $payment_gateway_entity) {    
    ...
    // Get the base field definitions for the order entity type.
    // $base_field_definitions = \Drupal::service('entity_field.manager')->getBaseFieldDefinitions('commerce_order');
    // $debug_msg = print_r($base_field_definitions, true);
    // $helper->log($debug_msg);

    // $this->updateOrderState($order, ($payment_state === 'completed') ? 'completed' : 'validation');    
  }
